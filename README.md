...
- `KanbanBoardState`
    - [ ] 태스크 추가 시 totalCount가 증가한다
    - [ ] Done 상태 태스크 추가 시 completeCount가 증가한다
    - [ ] 태스크가 없을 때 completeRatio는 0이다
    - [ ] 전체 태스크 중 Done 비율이 올바르게 계산된다

### UI 테스트

- 다이얼로그
    - [ ] 태스크 생성 버튼 클릭 시 다이얼로그이 노출된다
    - [ ] 취소 버튼 클릭 시 다이얼로그이 닫힌다

- 칸반 보드
    - [ ] 태스크 생성 완료 시 해당 상태 컬럼에 태스크가 노출된다
    - [ ] 태스크 생성 완료 시 SnackBar가 노출된다
